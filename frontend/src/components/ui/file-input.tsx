...
export function FileInput({ label, accept, multiple, onChange, error }: FileInputProps) {
  return (
    <div className={`relative border-2 border-dashed rounded-[2rem] p-8 text-center transition-all group cursor-pointer ${error ? "border-destructive/50 bg-destructive/5" : "border-orange-500/20 hover:bg-orange-500/5"}`}>
      ...
      />
      <Upload className={`h-8 w-8 mx-auto mb-2 transition-all ${error ? "text-destructive" : "text-muted-foreground group-hover:text-orange-500 group-hover:scale-110"}`} />
      <p className={`text-sm font-bold transition-colors ${error ? "text-destructive" : "text-muted-foreground group-hover:text-orange-500"}`}>{label}</p>
    </div>
  )
}
