...
def filtrar_falta_contador_csv(
    ...
    datos = datos[datos["Tipo"].str.contains("FALTA CONTADOR", na=False, case=False)].copy()
    if datos.empty:
        raise ValueError("No se encontraron filas con Tipo == 'FALTA CONTADOR'.")

    # Transformación similar a la original
    rename_map = {"Nro_serie": "SERIE", "FechaTomaContadorAnterior1": "FECHA", "ImpreContadorAnterior": "CONTADOR"}
    ...
    columnas_finales = ["SERIE", "FECHA", "TIPO", "CLASE_10", "CONTADOR_10", "CLASE_20", "CONTADOR_20", "MOTIVO", "OBSERVACION"]
    for col in columnas_finales:
        if col not in datos.columns: datos[col] = ""
    
    datos = datos[columnas_finales].groupby(["SERIE", "FECHA", "TIPO"], as_index=False).agg({
        "CLASE_10": "first", "CONTADOR_10": "max", "CLASE_20": "first", "CONTADOR_20": "max", "MOTIVO": "first", "OBSERVACION": "first"
    })
...
def procesar_suma_fija(
    ...
    for archivo_xls in archivos_xls:
        ...
        if "Nro Serie" in datos.columns: datos.rename(columns={"Nro Serie": "SERIE"}, inplace=True)
...
# --- 3) AUTOESTIMACIÓN ---
def ejecutar_autoestimacion(ruta_csv_detalle: str, fecha_nueva: str, carpeta_salida: str) -> Tuple[str, str]:
    ...
    # Normalización
    if "Nro_serie" in df.columns: df["Nro_serie"] = df["Nro_serie"].astype(str).str.strip().str.upper()
    df["ContActual"] = pd.to_numeric(df["ContActual"], errors="coerce").fillna(0).astype(int)
    df["Impresiones_Realizadas"] = pd.to_numeric(df["Impresiones_Realizadas"], errors="coerce").fillna(0).astype(int)
    
    df["ContadorNuevo"] = df["ContActual"] + df["Impresiones_Realizadas"]
    df["FechaNueva"] = fecha_nueva

    # CSV 1: Importación
    df_import = df[["Nro_serie", "FechaNueva", "ContadorNuevo", "ContActual", "Impresiones_Realizadas"]].copy()
...
def calcular_estimacion_manual(ci: int, cf: int, fi_str: str, ff_str: str, fe_str: str):
    ...
    if ndias <= 0: raise ValueError("Rango de días inválido")
