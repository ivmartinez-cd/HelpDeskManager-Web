...
def calcular_resultado_estimacion(
    contador_final: int, impresiones_diarias: float, dias_estimacion: int
):
    if contador_final < 0:
        raise ValueError(f"contador_final no puede ser negativo: {contador_final}")
    if impresiones_diarias < 0:
        raise ValueError(
            f"impresiones_diarias no puede ser negativa: {impresiones_diarias}"
        )
    ...
    contador_estimado = math.ceil(
        contador_final + (impresiones_diarias * dias_estimacion)
    )
