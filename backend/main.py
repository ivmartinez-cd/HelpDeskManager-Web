...
# Limpieza de arranque: borrar cualquier subdirectorio residual en uploads
for item in UPLOAD_DIR.iterdir():
    if item.is_dir():
        try:
            shutil.rmtree(item)
            print(f"Limpieza de arranque: {item} eliminado")
        except Exception as e:
            print(f"No se pudo eliminar {item} en el arranque: {e}")
...
async def create_ftp_client(client_data: ClientSchema, db: Session = Depends(get_db)):
    ...
    try:
        ...
        existing = (
            db.query(FTPClient).filter(FTPClient.name == client_data.name).first()
        )
        if existing:
...
async def update_ftp_client(
    client_id: int, client_data: ClientSchema, db: Session = Depends(get_db)
):
...
async def startup_seed():
    ...
    try:
        ...
        if db.query(ResourceLink).count() == 0:
            demo_links = [
                ResourceLink(
                    name="Manual App Mobile",
                    url="https://cdst-ar.github.io/ST/appmobile",
                    category="Manuales",
                ),
                ResourceLink(
                    name="Instructivos Contadores/ST",
                    url="https://sites.google.com/view/calendarict",
                    category="Documentación",
                ),
                ResourceLink(
                    name="Manuales Impresoras",
                    url="https://drive.google.com/drive/folders/backup",
                    category="Otros",
                ),
                ResourceLink(
                    name="Envíos Logística",
                    url="https://docs.google.com/spreadsheets/logs",
                    category="Otros",
                ),
            ]
...
async def process_ftp_client(
    ...
    try:
        ...
        if not client:
            raise HTTPException(
                status_code=404,
                detail=f"Cliente '{client_name}' no encontrado en la base de datos.",
            )
...
async def stc_process_db3(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        if count == 0:
            raise HTTPException(
                status_code=400,
                detail="No se encontraron IPs válidas en los archivos proporcionados.",
            )
...
async def stc_process_txt(file: UploadFile = File(...)):
    ...
    try:
        ...
        if count == 0:
            raise HTTPException(
                status_code=400,
                detail="No se encontraron IPs válidas en el archivo proporcionado.",
            )
...
async def tool_en0(
    file: UploadFile = File(...), fecha: str = Form(...), cliente: str = Form(...)
):
...
async def tool_suma_fija(
    files: List[UploadFile] = File(...), fecha: str = Form(...), hojas: int = Form(...)
):
    """Suma Fija de hojas."""
    try:
        temp_paths = []
...
async def tool_proyeccion(
    ...
    try:
        ...
        out_excel, out_csv, logs, summary, data, validation, audit = ejecutar_proyeccion(
...
async def process_db3(
    files: List[UploadFile] = File(...), fecha_maxima: str = Form("")
):
    """Procesa uno o varios archivos DB3 y devuelve un CSV consolidado con aislamiento por UUID."""
    ...
    temp_paths = []
    try:
        # Lógica para evitar colisiones dentro del mismo lote
        seen_filenames = {}

        for file in files:
            # Limpiar el nombre del archivo para evitar path traversal
            safe_name = re.sub(r"[^a-zA-Z0-9._-]", "_", file.filename)

            # Manejar duplicados en el mismo lote
            if safe_name in seen_filenames:
                seen_filenames[safe_name] += 1
                name_parts = safe_name.rsplit(".", 1)
                if len(name_parts) > 1:
                    current_name = (
                        f"{name_parts[0]}({seen_filenames[safe_name]}).{name_parts[1]}"
                    )
                else:
                    current_name = f"{safe_name}({seen_filenames[safe_name]})"
            else:
                seen_filenames[safe_name] = 0
                current_name = safe_name

            temp_path = session_dir / current_name
            print(f"DEBUG [{session_id}]: Guardando archivo {current_name}")

            with temp_path.open("wb") as buffer:
                shutil.copyfileobj(file.file, buffer)
            temp_paths.append(str(temp_path))

        # Usar el nombre del primer archivo como base para el resultado
        base_name = re.sub(r"[^a-zA-Z0-9._-]", "_", files[0].filename).replace(
            ".db3", ""
        )
        if len(files) > 1:
            base_name += f"_batch_{len(files)}"

        # El script devuelve la ruta completa y la lista de advertencias
        output_file_path, warnings = procesar_db_a_csv(
            archivos_db=temp_paths,
            fecha_maxima=fecha_maxima,
            nombre_base_salida=base_name,
            carpeta_salida=str(OUTPUT_DIR),
        )

        print(f"DEBUG [{session_id}]: Archivo generado en {output_file_path}")
        if warnings:
            print(f"DEBUG [{session_id}]: Advertencias: {warnings}")

        output_path = Path(output_file_path)
        if not output_path.exists():
            raise HTTPException(
                status_code=500,
                detail="Error: El archivo procesado no se encontró en el disco.",
            )

        headers = {}
        if warnings:
            # Enviamos las advertencias en una cabecera para que el frontend pueda verlas si lo desea
        ...
        print(f"ERROR [{session_id}] durante el procesamiento:\n{error_trace}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        # Limpieza total de la carpeta de sesión
...
async def ers_clients_list():
    ...
    try:
        ...
        import traceback
        print(f"ERROR obteniendo clientes ERS:\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def ers_process_meters(
    ...
    try:
        ...
        import traceback
        print(f"ERROR procesando contadores ERS:\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))
